...
object CanBuildFromDeserializer {
  def forVector[Elem](implicit elemDeser: Deserializer[Elem]): CanBuildFromDeserializer[Elem, Vector[Elem]] = {
    val cbf: Factory[Elem, Vector[Elem]] = implicitly[Factory[Elem, Vector[Elem]]]
    new CanBuildFromDeserializer[Elem, Vector[Elem]]()(cbf, elemDeser)
  }
}
...
final class CanBuildFromDeserializer[Elem,Col](implicit cbf: Factory[Elem,Col], elemDeser: Deserializer[Elem]) extends CollectionDeserializerBase[Col] {
  ...
  protected def readCollection(input: CollectionInput): Col = {
    val builder: Builder[Elem,Col] = cbf.newBuilder
