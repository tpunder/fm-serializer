...
import fm.serializer.json.{JSON, JSONDeserializerOptions, TestJSON}
...
final class TestJackson extends TestJSON {
  ...
  def makeInput(json: String): JsonParserInput = new JsonParserInput(Json.jsonFactory.createParser(json), JSONDeserializerOptions.default)
